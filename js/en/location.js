...
(() => {
  ...
  async function loadMapDependencies() {
    await Promise.all(leafletResources.styles.map(loadStylesheet));
    for (const src of leafletResources.scripts) await loadScript(src);
    if (!window.L || typeof window.L.markerClusterGroup !== "function") {
      throw new Error("Leaflet dependencies unavailable");
    }
  }
  ...
  function buildCard(store) {
    ...
    card.innerHTML = `
      ...
      <div class="location-store-actions">
        ...
        <a target="_blank" rel="noopener noreferrer" href="${escapeHtml(store.mapsUrl)}">Directions</a>
      </div>`;
  ...
  function useMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(position => {
      ...
      locateBtn.disabled = false;
      window.alert("Location could not be accessed. Please allow location permission in your browser.");
    }, {
  ...
  const ensureStarted = () => {
    ...
    startPromise = loadMapDependencies()
      .then(() => {
        start();
      ...
      .catch((error) => {
        console.error("HaloGo locator map failed to load:", error);
        mapElement.removeAttribute("aria-busy");
        mapElement.classList.remove("is-loading");
        mapElement.classList.add("is-load-error");
        mapElement.innerHTML = '<div class="location-map-load-error"><strong>Peta tidak dapat dimuatkan.</strong><span>Cuba muat semula halaman untuk mengaktifkan peta dan carian lokasi.</span></div>';
        startPromise = null;
      });
    return startPromise;
  };
  ...
  heroSearchForm?.addEventListener("submit", (event) => {
    ...
    document.getElementById("locator")?.scrollIntoView({ behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth", block: "start" });
    ensureStarted().then(() => {
      if (!map) return;
      searchInput.value = query;
      render();
      window.setTimeout(() => searchInput.focus({ preventScroll: true }), 120);
    });
  });

  const warmMap = () => ensureStarted();
  if ("IntersectionObserver" in window && locatorSection) {
    const observer = new IntersectionObserver((entries, currentObserver) => {
      if (!entries.some(entry => entry.isIntersecting)) return;
      currentObserver.disconnect();
      warmMap();
    }, { rootMargin: "700px 0px", threshold: 0.01 });
    observer.observe(locatorSection);
  } else {
    window.addEventListener("load", warmMap, { once: true });
  }

  window.addEventListener("load", () => {
    const schedule = window.requestIdleCallback || ((callback) => window.setTimeout(callback, 900));
    schedule(() => { if (!map && !startPromise) warmMap(); }, { timeout: 2600 });
  }, { once: true });
})();
