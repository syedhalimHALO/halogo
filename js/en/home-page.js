...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setActive(index, options) {
    ...
    if (settings.scrollMobile && mobileTrack && mobileFeatures[activeIndex] && window.innerWidth <= 980) {
      isProgrammaticScroll = true;

      const activeFeature = mobileFeatures[activeIndex];
      const targetLeft = Math.max(
        0,
        activeFeature.offsetLeft - (mobileTrack.clientWidth - activeFeature.offsetWidth) / 2
      );
