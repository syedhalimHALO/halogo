...
(() => {
  ...
  ready(() => {
    ...
    document.querySelectorAll("[data-plan-finder]").forEach((finder) => {
      ...
      const labels = {
        all: "All 6 plans are displayed.",
        budget: "2 budget-friendly plans found: EpikCall+20 and Epik+28.",
        data: "4 high-data plans found, starting from Epik+35.",
        roaming: "3 plans with e-Roaming found.",
        family: "2 family plans found: Epik+ Family Safe and Epik+ Family."
      };
    ...
    if (assistant && toggle && panel) {
      ...
      options.forEach((option) => {
        const text = option.dataset.message || "Hello Team Halo. I need some assistance.";
      ...
      const openPanel = () => {
        panel.hidden = false;
        toggle.setAttribute("aria-expanded", "true");
        toggle.setAttribute("aria-label", "Close Team Halo WhatsApp support");
      ...
      const closePanel = (returnFocus = false) => {
        panel.hidden = true;
        toggle.setAttribute("aria-expanded", "false");
        toggle.setAttribute("aria-label", "Open Team Halo WhatsApp support");
        if (returnFocus) toggle.focus();
      };
